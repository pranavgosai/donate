  ...
  return hre.ethers.utils.formatEther(balanceBigInt);
   ...
   async function consoleMemos(memos){
    for(const  memo of memos ){
      ...
      console.log(`At ${timestamp},name ${name},address ${from},message ${massage}`);
...
console.log("address of contreact:", contract.address);
...
await contract.connect(from1).buyChai("from1", "very good",amount);
await contract.connect(from2).buyChai("from2", "very good",amount);
